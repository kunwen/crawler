...
class LangagesofFamily(object):
    ...
    def translate(self, inputFile, outputFile, lik, ssize):  
        ...
        line = fin.strip().decode(chardet.detect(fin).get('encoding'), 'ignore')     #去除每行的首位空格等，并统一转化成Unicode  
        ...
        if lineTuple[0] in lik[0] or lineTuple[0] in self.countrylist:   #如果该行语言大部分为中文  
            ...
            p = re.compile(r'[\n]+')
            with open(outputFile, 'w') as fout:                                  #以写的方式打开输出文件  
